...
+ (NSString *)formatMoney:(NSString *)money{
    ...
    NSNumber *num = [format numberFromString:money];
    
    format.numberStyle = NSNumberFormatterCurrencyStyle;
    [format setGeneratesDecimalNumbers:YES];
    
    NSString *targetStr  = [format stringFromNumber:num];
    NSCharacterSet *set = [[NSCharacterSet characterSetWithCharactersInString:@"1234567890.,"] invertedSet];
    targetStr = [targetStr stringByTrimmingCharactersInSet:set];
    return targetStr;
}
#pragma mark ==========================================
